...
ckpt_file_en = os.environ.get("CKPT_FILE_GEN")
vocab_file_en = os.environ.get("VOCAB_FILE_GEN")
...
ema_model_gen = load_model(model_cls, model_cfg.arch, ckpt_file, mel_spec_type=vocoder_name, vocab_file=vocab_file, use_ema=use_ema)
ema_model_en = load_model(model_cls, model_cfg.arch, ckpt_file_en, mel_spec_type=vocoder_name, vocab_file=vocab_file_en, use_ema=True)
...
if __name__ == "__main__":
    main("Per ottimizzare i processi logistici e garantire l'integrità dei prodotti durante tutte le fasi della movimentazione, il trasporto delle merci viene effettuato su pallet standardizzati, che permettono una gestione più rapida ed efficiente dello stoccaggio, del carico e dello scarico, riducendo al minimo il rischio di danneggiamenti e migliorando la tracciabilità lungo l'intera filiera distributiva.", "it")